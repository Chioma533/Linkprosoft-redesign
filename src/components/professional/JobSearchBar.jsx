...
const JobSearchBar = ({ onApply }) => {
  ...
  const FilterDropdown = ({
    ...
    <div
      className={`relative shrink-0 ${openDropdown === name ? "z-40" : "z-10"}`}
    >
      <button
        id={id}
        onClick={() => toggle(name)}
      ...
      >
        {Icon && (
          <Icon className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-400 shrink-0" />
        )}
  ...
  return (
    <div
      ref={barRef}
      className="relative z-30 flex items-center gap-2 overflow-x-auto md:overflow-visible flex-nowrap w-full py-1 sm:py-2 hide-scrollbar"
    >
      ...
      {/* Apply button — hidden on mobile to match the mockup layout */}
      <button
        ...
        className="hidden sm:inline-flex px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-xs sm:text-sm rounded-full transition-all duration-200 cursor-pointer whitespace-nowrap shrink-0 min-h-[36px] sm:min-h-[42px]"
      >
        Apply
      </button>
    </div>
