...
tools:
  allow:
    ...
    - git
    - terminal
    - http (for Figma API only, requires user-provided token)
  forbid:
...
inputs:
  ...
  optional:
    - framework: "target project framework. default: react+tailwind"
...
workflow: |
  ...
  2. Fetch styles (colors, text styles) via Figma API (styles endpoint + file nodes as needed).
  3. Normalize and map tokens: produce `--color-...` CSS vars and a `tailwind.config.js` snippet.
  4. Create `src/styles/themes.css` with CSS variables for `default`, `dark`, and `white` themes.
  5. Add a `ThemeProvider` (`src/context/ThemeProvider.jsx`) and a `ThemeToggle` component.
  6. Wire persistence (`localStorage`) and prefer OS-level `prefers-color-scheme` on first load.
  7. Provide a migration patch and README with usage and verification steps.
outputs:
  - files:
